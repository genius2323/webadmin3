...
<div class="container-fluid px-4">
    ...
    <div class="card mb-4">
        <div class="card-body">
            <h4 class="mb-3">Otorisasi Daya</h4>
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 gap-2">
                <form method="get" class="d-flex align-items-center gap-2 mb-0">
                    <input type="text" name="search" class="form-control form-control-sm" placeholder="Cari daya..." value="<?= esc($search ?? '') ?>" style="max-width:180px;">
                    ...
                    <?php if (!empty($search)): ?>
                        <a href="<?= site_url('otorisasi_klasifikasi/otorisasi_daya') ?>" class="btn btn-sm btn-outline-secondary d-flex align-items-center justify-content-center" title="Clear" style="height:32px; width:32px; padding:0;"><i data-feather="x"></i></a>
                    <?php endif; ?>
                </form>
                <form method="get" class="d-flex align-items-center gap-2 mb-0">
                ...
                <table class="table table-bordered table-hover align-middle mb-0 table-radius">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th style="text-align:center;">Nama Daya</th>
                    ...
                    <tbody>
                        <?php if (!empty($dayas)): ?>
                            <?php foreach ($dayas as $i => $daya): ?>
                                <tr>
                                    ...
                                    <td style="text-align:center;">
                                        <form method="post" action="<?= site_url('otorisasi_klasifikasi/otorisasi_daya/setOtorisasiDaya') ?>">
                                            <input type="hidden" name="daya_id" value="<?= $daya['id'] ?>">
                                            <input type="hidden" name="otoritas" value="<?= ($daya['otoritas'] ?? null) === 'T' ? 'F' : 'T' ?>">
                                            <button type="submit" class="btn btn-sm <?= ($daya['otoritas'] ?? null) === 'T' ? 'btn-danger' : 'btn-success' ?>">
                                                <?= ($daya['otoritas'] ?? null) === 'T' ? 'Nonaktifkan' : 'Otorisasi' ?>
                                            </button>
                                        </form>
                                    </td>
                                </tr>
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="4" class="text-center">Tidak ada data daya.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
