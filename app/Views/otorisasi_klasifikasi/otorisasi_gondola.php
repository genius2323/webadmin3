<?php
// View: app/Views/otorisasi_klasifikasi/otorisasi_gondola.php
?>
...
<div class="container-fluid px-4">
    ...
    <div class="card animate__animated animate__fadeInUp mb-4">
        <div class="card-body">
            <h4 class="mb-3">Otorisasi Gondola</h4>
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 gap-2">
                <form method="get" class="d-flex align-items-center gap-2 mb-0">
                    <input type="text" name="search" class="form-control form-control-sm" placeholder="Cari gondola..." value="<?= esc($search ?? '') ?>" style="max-width:180px;">
                    ...
                    <?php if (!empty($search)): ?>
                        <a href="<?= site_url('otorisasi_klasifikasi/otorisasi_gondola') ?>" class="btn btn-sm btn-outline-secondary d-flex align-items-center justify-content-center" title="Clear" style="height:32px; width:32px; padding:0;"><i data-feather="x"></i></a>
                    <?php endif; ?>
                </form>
                <form method="get" class="d-flex align-items-center gap-2 mb-0">
                ...
                <table class="table table-bordered table-hover align-middle mb-0 table-radius">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th style="text-align:center;">Nama Gondola</th>
                    ...
                    <tbody>
                        <?php if (!empty($gondolas)): ?>
                            <?php foreach ($gondolas as $i => $gondola): ?>
                                <tr>
                                    <td style="text-align:center;"> <?= ($pager->getCurrentPage() - 1) * $pager->getPerPage() + $i + 1 ?> </td>
                                    <td><?= esc($gondola['name']) ?></td>
                                    <td style="text-align:center;">
                                        <?php if (($gondola['otoritas'] ?? null) === 'T'): ?>
                                    ...
                                    <td style="text-align:center;">
                                        <form method="post" action="<?= site_url('otorisasi_klasifikasi/otorisasi_gondola/setOtorisasiGondola') ?>">
                                            <input type="hidden" name="gondola_id" value="<?= $gondola['id'] ?>">
                                            <input type="hidden" name="otoritas" value="<?= ($gondola['otoritas'] ?? null) === 'T' ? 'F' : 'T' ?>">
                                            <button type="submit" class="btn btn-sm <?= ($gondola['otoritas'] ?? null) === 'T' ? 'btn-danger' : 'btn-success' ?>">
                                                <?= ($gondola['otoritas'] ?? null) === 'T' ? 'Nonaktifkan' : 'Otorisasi' ?>
                                            </button>
                                        </form>
                                    </td>
                                </tr>
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="4" class="text-center">Tidak ada data gondola.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
