...
<div class="page-header" style="display:flex;align-items:center;gap:16px;margin-left:24px;margin-bottom:18px;">
    <div class="page-header-icon" style="display:flex;align-items:center;">
        <span class="material-symbols-outlined" style="font-size:2.2rem;">settings_input_component</span>
    </div>
    <div>
        <h1 class="page-header-title" style="margin:0;">Master Fiting</h1>
        <p class="page-header-subtitle" style="margin:0;">Kelola seluruh data fiting di sini.</p>
    </div>
</div>
<div class="content-card">
    <div class="form-group" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px; margin-bottom: 20px;">
        <form method="get" style="display: flex; gap: 6px; align-items: center;">
            <input type="text" name="keyword" class="input-m3-date" placeholder="Cari fiting..." value="<?= esc($_GET['keyword'] ?? '') ?>" style="height:32px; font-size:0.95rem; padding:6px 10px; border-radius:8px;">
        ...
        <a href="<?= site_url('masterfiting/create') ?>" class="btn-m3 btn-m3-sm" style="height:32px; min-width:32px; padding:6px 10px; border-radius:8px; text-decoration:none;">
    ...
    <div class="table-responsive">
        <table class="table-m3">
            ...
            <tbody>
                <?php if (!empty($fiting)): ?>
                    <?php foreach ($fiting as $i => $row): ?>
                        <tr>
                            <td style="text-align:center;"><?= $i + 1 ?></td>
                            <td style="text-align:center;"><?= esc($row['name']) ?></td>
                            <td style="text-align:center;"><?= esc($row['description']) ?></td>
                            ...
                            <td style="min-width:60px;display:flex;gap:4px;justify-content:center;align-items:center;">
                                <?php if (!empty($row['otoritas']) && $row['otoritas'] === 'T'): ?>
                                    <a href="<?= site_url('masterfiting/edit/' . $row['id']) ?>" class="btn-m3 btn-m3-sm btn-m3-warning" style="height:28px; min-width:28px; padding:4px 8px; border-radius:8px; text-decoration:none; background:#FFD600; color:#333;">
                                        <span class="material-symbols-outlined" style="font-size:16px;">edit</span>
                                    </a>
                                    <a href="<?= site_url('masterfiting/delete/' . $row['id']) ?>" class="btn-m3 btn-m3-sm btn-m3-danger" style="height:28px; min-width:28px; padding:4px 8px; border-radius:8px; text-decoration:none; background:#e53935; color:#fff;" onclick="return confirm('Yakin ingin menghapus data ini?')">
