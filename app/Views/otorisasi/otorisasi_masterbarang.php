        ...
        <div class="page-header-content">
            <div class="row align-items-center justify-content-between pt-3">
                <div class="col-auto mb-3">
                    ...
                    <div class="small">Kelola otorisasi akses data master barang</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 gap-2">
                <form method="get" class="d-flex align-items-center gap-2 mb-0">
                    ...
                    <?php if (!empty($search)): ?>
                        <a href="<?= site_url('otorisasi/otorisasi_masterbarang') ?>" class="btn btn-sm btn-outline-secondary d-flex align-items-center justify-content-center" title="Clear" style="height:32px; width:32px; padding:0;"><i data-feather="x"></i></a>
                    <?php endif; ?>
                </form>
                <form method="get" class="d-flex align-items-center gap-2 mb-0">
            ...
            <div class="table-responsive">
                ...
                <table class="table table-bordered table-hover align-middle mb-0 table-radius">
                    ...
                    <tbody>
                        <?php if (!empty($list)): ?>
                            <?php foreach ($list as $i => $barang): ?>
                                <tr>
                                    <td style="text-align:center;"> <?= ($pager->getCurrentPage() - 1) * $pager->getPerPage() + $i + 1 ?> </td>
                                    <td><?= esc($barang['kode'] ?? '-') ?></td>
                                    <td><?= esc($barang['name'] ?? '-') ?></td>
                                    <td><?= esc($barang['category_id'] ?? '-') ?></td>
                                    <td style="text-align:center;">
                                        <?php if (($barang['otoritas'] ?? null) === 'T'): ?>
                                    ...
                                    <td style="text-align:center;">
                                        <form method="post" action="<?= site_url('otorisasi/otorisasi_masterbarang/setOtorisasiMasterBarang') ?>">
                                            <input type="hidden" name="barang_id" value="<?= $barang['id'] ?>">
                                            <input type="hidden" name="otoritas" value="<?= ($barang['otoritas'] ?? null) === 'T' ? 'F' : 'T' ?>">
                                            <button type="submit" class="btn btn-sm <?= ($barang['otoritas'] ?? null) === 'T' ? 'btn-danger' : 'btn-success' ?>">
                                                <?= ($barang['otoritas'] ?? null) === 'T' ? 'Nonaktifkan' : 'Otorisasi' ?>
                                            </button>
                                        </form>
                                    </td>
                                </tr>
