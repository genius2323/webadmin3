            ...
            <div class="row align-items-center justify-content-between pt-3">
                <div class="col-auto mb-3">
                    <h1 class="page-header-title">
                        ...
                        Otorisasi Master Sales
                    </h1>
                    <div class="small">Kelola otorisasi akses master sales</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 gap-2">
                <form method="get" class="d-flex align-items-center gap-2 mb-0">
                    <input type="text" name="search" class="form-control form-control-sm" placeholder="Cari sales..." value="<?= esc($search ?? '') ?>" style="max-width:180px;">
                    ...
                    <?php if (!empty($search)): ?>
                        <a href="<?= site_url('otorisasi/otorisasi_sales') ?>" class="btn btn-sm btn-outline-secondary d-flex align-items-center justify-content-center" title="Clear" style="height:32px; width:32px; padding:0;"><i data-feather="x"></i></a>
                    <?php endif; ?>
                </form>
                <form method="get" class="d-flex align-items-center gap-2 mb-0">
                ...
                <table class="table table-bordered table-radius align-middle mb-0">
                    ...
                    <tbody>
                        <?php foreach ($list as $sales): ?>
                            <tr>
                                <td><?= esc($sales['kode'] ?? '') ?></td>
                                <td style="white-space:nowrap;"><?= esc($sales['nama'] ?? '') ?></td>
                                <td style="text-align:center;">
                                    <?php if (($sales['otoritas'] ?? '') === 'T'): ?>
                                ...
                                <td style="text-align:center;">
                                    <form action="<?= site_url('otorisasi/otorisasi_sales/setOtorisasiSales') ?>" method="post" class="d-inline">
                                        <?= csrf_field() ?>
                                        <input type="hidden" name="sales_id" value="<?= $sales['id'] ?>">
                                        <input type="hidden" name="otoritas" value="<?= ($sales['otoritas'] ?? '') === 'T' ? 'F' : 'T' ?>">
                                        <button type="submit" class="btn btn-sm btn-<?= ($sales['otoritas'] ?? '') === 'T' ? 'danger' : 'success' ?>">
                                            <?= ($sales['otoritas'] ?? '') === 'T' ? 'Nonaktifkan' : 'Otorisasi' ?>
                                        </button>
                                    </form>
                                </td>
                            </tr>
